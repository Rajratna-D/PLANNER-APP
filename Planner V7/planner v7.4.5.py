...
# ── Sound alert (stdlib winsound on Windows, fallback silent on other OS) ─
try:
    import winsound
    HAS_SOUND = True
except ImportError:
...
# Palette
BG        = "#0B0E14"   # Premium deep space
PANEL     = "#131722"   # Sidebar and topbar
CARD      = "#1A1F2C"   # Main card background
CARD2     = "#212836"   # Secondary cards / dropdowns
HOVER     = "#2A3345"   # Hover states
BORDER    = "#273142"   # Subtle borders
BORDER2   = "#3B4A66"   # Focused / active borders
...
P_BG    = {"Immediate": "#2C1216", "Important": "#2C1E0A",
           "2nd Priority": "#0D1B2A", "3rd Priority": "#0A2016", "Someday": "#171A24"}
...
THEMES = {
    "dark": {
        "BG": "#0B0E14", "PANEL": "#131722", "CARD": "#1A1F2C", "CARD2": "#212836",
        "HOVER": "#2A3345", "BORDER": "#273142", "BORDER2": "#3B4A66",
        "TEXT": "#F8FAFC", "SUBTEXT": "#94A3B8", "DIM": "#475569",
    },
    "light": {
        "BG": "#F4F6F9", "PANEL": "#EAEFF4", "CARD": "#FFFFFF", "CARD2": "#F8FAFC",
        "HOVER": "#E2E8F0", "BORDER": "#CBD5E1", "BORDER2": "#94A3B8",
...
def _get_modern_font():
    _avail = set(__import__("tkinter.font", fromlist=["families"]).families())
    _fonts = ["Inter", "Roboto", "Outfit", "Segoe UI", "SF Pro Display", "Helvetica Neue", "Arial"]
    return next((f for f in _fonts if f in _avail), "Arial")
...
def mk_label(parent, text, size=12, bold=False, color=TEXT, bg=None, **kw):
    ...
    return ctk.CTkLabel(parent, text=text,
                        font=ctk.CTkFont(family=_get_modern_font(), size=size, weight=weight),
...
def mk_btn(parent, text, cmd, color=BORDER2, text_color=TEXT,
    ...
    font_kw = {"family": _get_modern_font(), "size": size, "weight": "bold"}
...
class Card(ctk.CTkFrame):
    ...
    def __init__(self, parent, accent_color=BORDER, **kw):
        ...
        self._body.pack(fill="both", expand=True, padx=20, pady=(12, 16))

        # Add interactive hover states
        for w in (self, self._bar, self._body):
...
class PomoAlert:
    """
    Full-screen-overlay Pomodoro alert.
    - Cannot be dismissed by clicking away
    - Plays a sound alert (Windows winsound, graceful fallback)
    - Stays on screen until user actively clicks START BREAK / START FOCUS
    - Shows session number, what comes next, and the task that was worked on
    """

    def __init__(self, root, session_type, session_num, next_type,
                 next_mins, task_label=""):
        self._root = root
        ...
        if session_type == "work":
            accent   = PINK
            icon_txt = "🍅"
            title    = f"FOCUS SESSION #{session_num} COMPLETE"
            msg      = f"You locked in. Time for a {next_mins}-minute break."
            btn_lbl  = f"▶  START {next_mins}-MIN BREAK"
            btn_col  = GREEN
        elif session_type == "long_break":
            accent   = GREEN
            icon_txt = "🌿"
            title    = "LONG BREAK COMPLETE"
            msg      = "Recharged. Time to get back to work."
            btn_lbl  = "▶  START FOCUS SESSION"
            btn_col  = PINK
        else:
            accent   = CYAN
            icon_txt = "☕"
            title    = "SHORT BREAK COMPLETE"
            msg      = "Break is over. Lock back in."
            btn_lbl  = "▶  START FOCUS SESSION"
            btn_col  = PINK
        ...
        def _play_sound():
            if HAS_SOUND:
                try:
                    # Three short beeps — hard to miss
                    for _ in range(3):
        ...
        # ── Window ────────────────────────────────────────────────────────
        self._win = tk.Toplevel(root)
        self._win.title("Pomodoro Alert")
        self._win.configure(bg=BG)
        self._win.attributes("-topmost", True)
        self._win.resizable(False, False)

        # Block interaction with main window until dismissed
        self._win.grab_set()
        ...
        # Prevent closing with X button — must use the action button
        self._win.protocol("WM_DELETE_WINDOW", lambda: None)

        # ── Layout ────────────────────────────────────────────────────────
        # Top accent bar
        tk.Frame(self._win, bg=accent, height=5).pack(fill="x")

        # Icon
        tk.Label(self._win, text=icon_txt,
                 font=("Segoe UI", 52), bg=BG, fg=accent).pack(pady=(28, 0))

        # Title
        tk.Label(self._win, text=title,
                 font=("Segoe UI", 18, "bold"),
                 bg=BG, fg=TEXT, wraplength=460).pack(pady=(10, 0))

        # Message
        tk.Label(self._win, text=msg,
                 font=("Segoe UI", 12),
                 bg=BG, fg=SUBTEXT, wraplength=460).pack(pady=(6, 0))

        # Task label (if set)
        if task_label and task_label not in ("", "(no task selected)"):
            tf = tk.Frame(self._win, bg=CARD)
            tf.pack(fill="x", padx=40, pady=(16, 0))
            tk.Frame(tf, bg=accent, height=2).pack(fill="x")
            tk.Label(tf, text=f"↳  {task_label}",
                     font=("Segoe UI", 10, "italic"),
                     bg=CARD, fg=accent,
                     wraplength=400).pack(padx=14, pady=10)

        # Divider
        tk.Frame(self._win, bg=BORDER, height=1).pack(
            fill="x", padx=40, pady=(20, 0))

        # Session counter
        tk.Label(self._win,
                 text=f"Session {session_num}  •  {date.today().strftime('%a %d %b')}",
                 font=("Segoe UI", 10),
                 bg=BG, fg=DIM).pack(pady=(10, 0))

        # Action button — only way to close
        btn = tk.Button(self._win,
                        text=btn_lbl,
        ...
        # Snooze — adds 2 more minutes before alerting again
        tk.Button(self._win,
                  text="⏰  Snooze 2 min",
                  font=("Segoe UI", 9),
                  bg=BG, fg=SUBTEXT,
                  relief="flat",
                  cursor="hand2",
                  command=self._snooze).pack(pady=(8, 0))

        # Repeat beep every 30 seconds until dismissed
        self._repeat_job = self._win.after(30000, self._repeat_beep)

    def _repeat_beep(self):
        """Beep again every 30s if still not dismissed."""
        def _play():
            if HAS_SOUND:
                try:
    ...
    def _snooze(self):
        """Close for now, reopen after 2 minutes."""
        try:
            if self._repeat_job:
                self._win.after_cancel(self._repeat_job)
        except: pass
        # Schedule re-alert
        self._root.after(120000, lambda: self._root.event_generate("<<PomoSnoozeEnd>>"))
...
class PlannerApp(ctk.CTk):
    ...
    def _refresh_overview(self):
        ...
        overdue = [x for x in pending_items if x[3] == "PAST"]
    ...
    def _pomo_session_done(self):
        ...
        self.data["pomodoro_log"].append({
            ...
            "duration_min": (self._pomo_work_secs() if session_type=="work" else
                 self._pomo_long_secs() if session_type=="long_break" else
                 self._pomo_short_secs())//60
        })
        save_data(self.data)
        ...
        if session_type == "work":
            ...
            # Determine break type and duration for alert
            if self._pomo_sessions % 4 == 0:
                next_type = "long_break"
                next_mins = self._pomo_long_secs() // 60
                ...
                next_type = "break"
                next_mins = self._pomo_short_secs() // 60
            ...
            # Show impossible-to-miss alert
            PomoAlert(self, "work", self._pomo_sessions, next_type,
                      next_mins, task_label=self.pomo_task_combo.get())
        else:
            next_mins = self._pomo_work_secs() // 60
            self._pomo_mode = "work"
            PomoAlert(self, session_type,
                self._pomo_sessions, "work",
                next_mins, task_label=self.pomo_task_combo.get())
            self._pomo_sessions, "work",
            next_mins, task_label=self.pomo_task_combo.get()
