...
class PlannerApp(ctk.CTk):
    ...
    def _show_tab(self, name):
        """Switch tabs with a smooth fade-in effect."""
    ...
    def _toggle_sidebar(self):
        ...
        if self._sidebar_open:
            self._nav_label.pack(anchor="w", padx=14, pady=(18,8))
            for name, r in self._sbtn_refs.items():
    ...
    def _animate_sidebar(self, target_w):
        """Animate sidebar width with ease-out."""
        ...
        new_w = max(self._sidebar_collapsed_w, min(self._sidebar_expanded_w, new_w))
        self.sidebar.configure(width=new_w)
        if new_w != target_w:
            self.after(12, lambda: self._animate_sidebar(target_w))
    ...
    def _refresh_overview(self):
        ...
        if syllabus:
            for subj in syllabus[:4]:
                ...
                pb = mk_progress(card.body, pct/100, VIOLET, BORDER, height=6)
    ...
    def _subject_card(self, parent, subj):
        ...
        # Progress bar
        pb = mk_progress(inn, pct/100, VIOLET, BORDER, height=6)
    ...
    def _pomo_update_display(self):
        ...
        goal = self.data.get("daily_goal", 6)
        if today_count == goal:
            show_toast(self, f"Daily goal reached! {goal} sessions 🔥", color=GOLD, icon="⬡")
    ...
    def _build_notes_tab(self):
        ...
        mk_btn(edit_top, "🗑 DELETE", self._delete_note, RED, TEXT, P_BG["Immediate"], 100, 34).pack(side="left")
    ...
    def _note_list_item(self, parent, note):
        ...
        mk_label(item, f"{subj}  •  {ts}", size=9, color=SUBTEXT).pack(anchor="w", padx=8)
        if preview:
            mk_label(item, preview + ("…" if len(note.get("body","")) > 50 else ""),
                     size=9, color=DIM).pack(anchor="w", padx=8, pady=(0,6))

        def _click(e=None, nid=note["id"]):
            self._note_selected = nid
            self._load_note(nid)
    ...
    def _refresh_productivity(self):
        ...
        # ══════════════════════════════════════════════════════════════════
        # ROW 4 — Monthly calendar heatmap
        # ══════════════════════════════════════════════════════════════════
        mk_label(root, "  MONTHLY HEATMAP", size=9, bold=True, color=SUBTEXT
                 ).pack(anchor="w", padx=28, pady=(6,4))
        cal_card = ctk.CTkFrame(root, fg_color=CARD, corner_radius=12)
        cal_card.pack(fill="x", padx=24, pady=(0,8))
        tk.Frame(cal_card, bg=VIOLET, height=2).pack(fill="x")

        cal_hdr = ctk.CTkFrame(cal_card, fg_color="transparent")
        cal_hdr.pack(fill="x", padx=18, pady=(10,6))
        mk_label(cal_hdr, today.strftime("%B %Y").upper(), size=12, bold=True, color=VIOLET
                 ).pack(side="left")
        # Legend
        for lbl3, col3 in [("EXCELLENT", GOLD),("GOOD", CYAN),("AVERAGE", ORANGE),
                            ("LOW", RED),("NONE", DIM)]:
            dot = ctk.CTkFrame(cal_hdr, fg_color=col3, width=8, height=8, corner_radius=4)
            dot.pack(side="right", padx=(0,2))
            mk_label(cal_hdr, lbl3, size=8, color=col3).pack(side="right", padx=(0,4))

        # Day-of-week headers
        dow_row = ctk.CTkFrame(cal_card, fg_color="transparent")
        dow_row.pack(fill="x", padx=18, pady=(0,4))
        for d_name in ["Mon","Tue","Wed","Thu","Fri","Sat","Sun"]:
            mk_label(dow_row, d_name, size=8, bold=True, color=SUBTEXT
                     ).pack(side="left", expand=True)

        # Calendar grid
        cal_grid = ctk.CTkFrame(cal_card, fg_color="transparent")
        cal_grid.pack(fill="x", padx=18, pady=(0,14))
        ...
        total_cells = first_dow + days_in_month.day
        rows_needed = (total_cells + 6) // 7

        for row_i in range(rows_needed):
            row_frame = ctk.CTkFrame(cal_grid, fg_color="transparent")
            row_frame.pack(fill="x", pady=2)
            for col_i in range(7):
                cell_num = row_i * 7 + col_i - first_dow + 1
                cell = ctk.CTkFrame(row_frame, fg_color="transparent", width=38, height=38)
                cell.pack(side="left", expand=True, padx=2)
                cell.pack_propagate(False)
                if 1 <= cell_num <= days_in_month.day:
                    cell_date = today.replace(day=cell_num).isoformat()
                    s = day_score(cell_date)
                    is_today = cell_num == today.day
                    if s >= 80:   bg_c = "#2A2200"; fg_c = GOLD
                    elif s >= 60: bg_c = "#082028"; fg_c = CYAN
                    elif s >= 40: bg_c = "#251500"; fg_c = ORANGE
                    elif s > 0:   bg_c = "#250810"; fg_c = RED
                    else:         bg_c = CARD2;     fg_c = DIM
                    border_c = PINK if is_today else bg_c
                    inner = ctk.CTkFrame(cell, fg_color=bg_c, corner_radius=6,
                                         border_width=2 if is_today else 0,
                                         border_color=border_c)
                    inner.pack(fill="both", expand=True)
                    mk_label(inner, str(cell_num), size=9,
                             bold=is_today, color=PINK if is_today else fg_c).pack(expand=True)
                else:
